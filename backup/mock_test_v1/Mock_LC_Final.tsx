...
export default function MockTest_LC_Set9({ onFinishLC }: Props) {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex flex-col h-screen bg-white overflow-hidden text-slate-900 select-none">
            ...
            <main className="flex-1 overflow-y-auto overflow-x-hidden relative flex bg-slate-100/50">
        ...
        case 4: // P3 Set 13 + P4 Sets 1-3 | P4 Sets 4-7
            return (
                <>
                    ...
                    <div className="booklet-page">
                        {renderP34Page(test9Part4.slice(0, 4), answers, onAnswer)}
                    </div>
                </>
            );
        case 5: // P4 Sets 8-10 | Empty (Wait for RC)
            return (
                <>
                    <div className="booklet-page">
                    ...
                    <div className="booklet-page">
                        <div className="flex h-full gap-8">
                            <div className="flex-1 flex flex-col gap-6">
                                {test9Part4.slice(8, 10).map(set => renderP34Set(set, answers, onAnswer))}
                            </div>
                            <div className="w-px bg-slate-100 my-4"></div>
                            <div className="flex-1 flex flex-col gap-6">
                                {/* Empty Column or reserved */}
                            </div>
                        </div>
                    </div>
                    <div className="booklet-page flex items-center justify-center bg-slate-50">
                        <div className="text-center opacity-30 select-none">
                            <span className="font-black text-6xl block mb-4 italic tracking-tighter">LISTENING END</span>
                            <p className="font-bold text-xl uppercase tracking-widest">Wait for RC Instructions</p>
                        </div>
                    </div>
                </>
            );
...
function renderP1Question(num: number, answers: any, onAnswer: any) {
    ...
    return (
        <div key={num} className="w-full">
            ...
            <div className="p1-photo-container">
                {qData?.image && <img src={qData.image} alt={`Q${num}`} className="w-full h-full object-cover" />}
