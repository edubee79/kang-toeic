...
export default function MockTest_LC_Set9({ onFinishLC }: Props) {
    ...
    const getSpreadQuestions = (idx: number): string[] => {
        ...
        if (idx === 2) {
            test9Part2.forEach(q => qIds.push(`p2_${q.id}`));
        ...
        if (idx === 4) {
            ...
            test9Part4.slice(0, 4).forEach(set => set.questions.forEach((q: any) => qIds.push(q.id)));
        }
        if (idx === 5) {
            test9Part4.slice(4, 10).forEach(set => set.questions.forEach((q: any) => qIds.push(q.id)));
        }
        return qIds;
    };
