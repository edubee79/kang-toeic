...
# Day 24 데이터 추출
day24 = df[df['Day'] == 24].copy()

print(f"=== Day 24 CSV 원본 데이터 ===")
print(f"총 단어 수: {len(day24)}")
print(f"\n첫 10개 단어:")
for idx, row in day24.head(10).iterrows():
    print(f"{row['No']:3d}. {row['Word']:20s} - {row['Meaning']}")

print(f"\n마지막 10개 단어:")
for idx, row in day24.tail(10).iterrows():
    print(f"{row['No']:3d}. {row['Word']:20s} - {row['Meaning']}")

# JSON 파일 확인
print(f"\n\n=== Day 24 JSON 파일 데이터 ===")
for part_num, part_name in [(1, 'headword'), (2, 'basic'), (3, 'intermediate'), (4, 'advanced')]:
    filename = f'day24_{part_num}_{part_name}.json'
    try:
        ...
        print(f"  마지막 3개: {data[-3]['word']}, {data[-2]['word']}, {data[-1]['word']}")
...
for part_num, part_name, start, end in [
    ...
    try:
        ...
        csv_part = day24[(day24['No'] >= start) & (day24['No'] <= end)]
        ...
        if mismatches:
            ...
            for m in mismatches[:10]:  # 처음 10개만 표시
                print(m)
        else:
            print(f"  ✅ 모든 단어 일치")
