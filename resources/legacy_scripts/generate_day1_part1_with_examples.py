...
# Day 1, Part 1 데이터 (1-40번)
day1_part1 = df[(df['Day'] == 1) & (df['No'] >= 1) & (df['No'] <= 40)].copy()

# TOEIC 스타일 예문 매핑 (수동 작성)
toeic_examples = {
...
# JSON 데이터 생성
part1_data = []
for idx, row in day1_part1.iterrows():
    no = int(row['No'])
    ex_en, ex_ko = toeic_examples.get(no, (f"Example for {row['Word']}.", f"{row['Meaning']} 예문."))
...
print(f"✅ day1_part1.json 생성 완료 (40 words)")
print(f"   첫 단어: {part1_data[0]['word']} - {part1_data[0]['exampleEn']}")
print(f"   마지막: {part1_data[-1]['word']} - {part1_data[-1]['exampleEn']}")
