...
# --- Day 1 Part 2 : No. 41 ~ 68 (28 words) ---
part2_df = df[(df['Day'] == 1) & (df['No'] >= 41) & (df['No'] <= 68)].copy()

print(f"Part 2 Range Check: Min {part2_df['No'].min()} ~ Max {part2_df['No'].max()}")
print(f"Part 2 Count: {len(part2_df)}")

if len(part2_df) != 28:
    raise ValueError(f"Part 2 count mismatch! Must be 28. Found {len(part2_df)}")

# Check Overlap with Part 1 (1-40)
if part2_df['No'].min() <= 40:
     raise ValueError("CRITICAL: Part 2 overlaps with Part 1 (No <= 40 detected)!")
...
for idx, row in part2_df.iterrows():
    no = int(row['No'])
    ex_en, ex_ko = toeic_examples_p2.get(no, (f"Example for {row['Word']}", f"{row['Meaning']} 예문"))
