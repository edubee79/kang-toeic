...
async function run() {
    ...
    for (const qSet of questionsRaw.sets) {
        ...
        const enrichedSet = {
            ...
            questionRange: range,
            audio: `/audio/ETS_TOEIC_3/Test_07/TEST 07_PART 3_${range}.mp3`,
        ...
        if (range === "62-64") {
            (enrichedSet as any).image = "/images/ETS_TOEIC_3/Test_07/Part_03/p3_q62.png";
        } else if (range === "65-67") {
            (enrichedSet as any).image = "/images/ETS_TOEIC_3/Test_07/Part_03/p3_q65.png";
        } else if (range === "68-70") {
            (enrichedSet as any).image = "/images/ETS_TOEIC_3/Test_07/Part_03/p3_q68.png";
        }

        finalSets.push(enrichedSet);
    }

    fs.writeFileSync('test07_part3_final.json', JSON.stringify(finalSets, null, 2));
    console.log(`✅ Finalized ${finalSets.length} sets for Part 3 Test 07.`);
}
