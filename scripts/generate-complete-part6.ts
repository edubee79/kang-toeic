...
function getQuestionText(q: any): string {
    if (q.text && q.text !== 'null') {
        return q.text;
    }
    // Generate default question text based on question type
    const firstOption = q.options[0]?.text || '';
    if (firstOption.length > 50) {
        return `Which sentence best fits in position ${q.id}?`;
    }
    return `What is the best word for position ${q.id}?`;
}
...
async function generateCompleteFile() {
    console.log('📚 Generating complete rc_part6.ts file...\n');
    ...
    for (const testNum of tests) {
        try {
            const passagesFile = `extracted_part6_test${testNum}.json`;
            const answersFile = `extracted_part6_test${testNum}_answers.json`;

            if (!fs.existsSync(passagesFile)) {
                console.log(`⚠️  Skipping Test ${testNum} - file not found`);
                continue;
            }
