...
input_file = 'src/data/toeic/reading/part5/tests.ts'
output_dir = 'src/data/toeic/reading/part5'

with open(input_file, 'r', encoding='utf-8') as f:
    content = f.read()

test_regex = re.compile(r'"testId":\s*(\d+)') # Note: Part 5 seems to use quotes for "testId"
...
for m in matches:
    ...
    with open(file_path, 'w', encoding='utf-8') as out_f:
        out_f.write(f"import {{ Part5TestSet }} from './types';\n\n")
        out_f.write(f"export const test{tid}Data: Part5TestSet = {raw_content};\n")
    
    extracted_tests.append(tid)

with open(input_file, 'w', encoding='utf-8') as f:
    f.write(f"import {{ Part5TestSet }} from './types';\n")
...
print(f"Successfully split and aggregated {len(extracted_tests)} Part 5 tests.")
