...
input_file = 'src/data/toeic/reading/part7/tests.ts'
...
# Track which tests we've extracted (only first unique one to avoid messy nesting)
seen_tids = set()
extracted_tests = []
...
for m in matches:
    ...
    with open(file_path, 'w', encoding='utf-8') as out_f:
        out_f.write(f"import {{ Part7Test }} from './types';\n\n")
        out_f.write(f"export const test{tid}Data: Part7Test = {raw_content};\n")
    
    extracted_tests.append(tid)

# Now update tests.ts to be the aggregator
with open(input_file, 'w', encoding='utf-8') as f:
    f.write(f"import {{ Part7Test }} from './types';\n")
...
print(f"Successfully split and aggregated {len(extracted_tests)} tests.")
