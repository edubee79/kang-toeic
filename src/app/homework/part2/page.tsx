...
import { ArrowLeft, Headphones, PlayCircle, Activity, Mic2, Lock, Layers, CheckCircle } from "lucide-react";
import { cn } from "@/lib/utils";
import { part2RealTests } from '@/data/toeic/listening/part2/tests';
...
export default function Part2Lobby() {
    ...
    useEffect(() => {
        ...
        const fetchAccess = async () => {
            ...
            if (userStr) {
                ...
                const units = [
                    ...Array.from({ length: 10 }, (_, i) => `Part2 Real Test Vol3 ${i + 1}`),
                    ...Array.from({ length: 10 }, (_, i) => `Part2 Real Test Vol4 ${i + 1}`)
                ];
    ...
    // Filter and unique volumes
    const volumes = Array.from(new Set(part2RealTests.map(t => t.vol))).sort();
    const filteredTests = part2RealTests.filter(t => t.vol === selectedVol);

    // NEW: Get limit for the specific volume
    const volLimits = access?.maxSets?.part2;
            ...
            <div className="w-full px-4 md:px-8 py-4 md:py-6">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-3 gap-2 md:gap-4 font-inter">
                    {filteredTests.map((test) => {
                        ...
                        const completionKey = `Part2 Real Test Vol${test.vol} ${test.testId}`;
                        const completion = completions[completionKey];

                        return (
                            <Card key={`${test.vol}-${test.testId}`}
                                className={cn(
                                    "group relative p-3 md:p-6 rounded-xl md:rounded-2xl border transition-all overflow-hidden flex flex-col gap-2 min-h-[100px] md:min-h-[120px]",
                                    !isLocked
                                ...
                                <div className="pl-9 md:pl-14 mt-auto">
                                    <p className={cn(
                                        ...
                                        {isLocked ? `VOL ${test.vol} ⸱ 25 Q` : (completion?.completed ? 'COMPLETED' : `PART 2 ⸱ VOL ${test.vol} ⸱ 25 Q`)}
                                    </p>
                                </div>
                            </Card>
                        );
