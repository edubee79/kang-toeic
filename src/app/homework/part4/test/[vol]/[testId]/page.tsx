...
import { Trophy, RotateCcw, AlertTriangle, CheckCircle2, BookOpen } from "lucide-react";
...
export default function Part4TestRunnerPage() {
    ...
    useEffect(() => {
        if (typeof window !== 'undefined') {
            setHasSavedProgress(!!localStorage.getItem(`p4_prog_v${vol}_t${testId}`));
        }
    }, [vol, testId]);
    ...
    const handleNext = () => {
        ...
        if (reviewMode) {
            if (currentIndex < reviewSets.length - 1) { setCurrentIndex(p => p + 1); window.scrollTo(0, 0); }
            else { setShowCompletion(true); }
            return;
        }
        const limit = testSets.length;
        if (currentIndex < limit - 1) { setCurrentIndex(p => p + 1); window.scrollTo(0, 0); }
