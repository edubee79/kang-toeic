...
import { part4RealTests, Part4Set, Part4Question } from '@/data/toeic/listening/part4/tests';
...
export default function Part4TestRunnerPage() {
    ...
    const [wrongQueue, setWrongQueue] = useState<Part4Question[]>([]);
    ...
    useEffect(() => {
        ...
        const found = part4RealTests.find(t => t.vol === vol && t.testId === testId);
        if (!found) { alert("Test not found"); router.push('/homework/part4'); return; }
        ...
        const fetchRetryAndLoad = async () => {
            if (retryMode && resultId) {
                ...
                const savedHistory = localStorage.getItem(`p4_hist_v${vol}_t${testId}`);
    ...
    useEffect(() => {
        ...
        const saved = localStorage.getItem(`p4_prog_v${vol}_t${testId}`);
        if (saved) {
            try {
    ...
    const handleSelect = (qId: string, label: string) => {
        ...
        localStorage.setItem(`p4_prog_v${vol}_t${testId}`, JSON.stringify({ currentIndex, selectedAnswers: newAns }));
        ...
        if (qElement) {
            ...
            if (container) {
                ...
                const currentIndex = questions.indexOf(qElement);
    ...
    const handleNext = () => {
        if (mode !== 'skim') return;
        const complete = testSets[currentIndex].questions.every(q => selectedAnswers[q.id]);
        if (!complete && !reviewMode) return;
        if (reviewMode) {
            if (currentIndex < testSets.length - 1) { setCurrentIndex(p => p + 1); window.scrollTo(0, 0); }
            else { setShowCompletion(true); setReviewMode(false); }
            return;
        }
        if (currentIndex < testSets.length - 1) { setCurrentIndex(p => p + 1); window.scrollTo(0, 0); }
        else { finishTest(selectedAnswers); }
    };
    ...
    const finishTest = async (finalAns = selectedAnswers) => {
        ...
        localStorage.removeItem(`p4_prog_v${vol}_t${testId}`);
            ...
            <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6">
                <div className="max-w-md w-full bg-slate-900 border border-slate-800 rounded-3xl p-8 shadow-2xl space-y-8">
                    <div className="text-center">
                        <p className="text-indigo-500 text-xs font-black uppercase tracking-widest mb-2">Part 4 · Vol {vol} · Test {testId}</p>
                    ...
                    <div className="space-y-4">
                        ...
                        <button onClick={() => setMode('skim')}
                            ...
                            <span className="text-xs text-indigo-200 uppercase tracking-wider">세트마다 {SKIM_TIME}초 스키밍 후 음원 재생</span>
                        </button>
                    </div>
                </div>
            </div>
            ...
            <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6">
                <div className="max-w-md w-full space-y-8 text-center">
                    <div className="animate-in zoom-in duration-500">
                        <div className="w-24 h-24 bg-indigo-500/10 rounded-3xl flex items-center justify-center mb-6 mx-auto ring-1 ring-indigo-500/50 shadow-2xl shadow-indigo-500/20">
                        ...
                        <p className="text-indigo-400 font-bold tracking-widest text-xs uppercase">ATTEMPT #{history.attempts} FINISHED</p>
                    </div>
                    ...
                    <div className="space-y-3">
                        <button onClick={handleRetake}
                            className="w-full h-14 bg-indigo-600 hover:bg-indigo-500 text-white font-black rounded-2xl shadow-xl transition-all active:scale-95 flex items-center justify-center gap-2">
            ...
            <div className="sticky top-0 z-10 bg-slate-950/95 backdrop-blur-md border-b border-white/5 shadow-2xl">
                <div className="max-w-3xl mx-auto px-4 py-3 flex items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-3">
                        ...
                        <button onClick={() => {
                            localStorage.setItem(`p4_prog_v${vol}_t${testId}`, JSON.stringify({ currentIndex, selectedAnswers }));
                    ...
                    <div className="sticky top-[116px] z-[5] py-4 mb-6 bg-slate-950/95 backdrop-blur-sm border-b border-white/5 -mx-4 px-4 md:-mx-8 md:px-8">
                        <div className="flex justify-center">
                            <div className="border border-slate-700 p-1 rounded-lg shadow-2xl bg-slate-900">
                                <img src={currentSet.image} alt="Graphic" className="max-h-60 md:max-h-80 object-contain rounded opacity-95 transition-all" />
                            </div>
                        </div>
                    </div>
                )}
                ...
                <div className="space-y-2 md:space-y-10">
                    {currentSet.questions.map((q) => (
                        <div key={q.id} id={q.id} className={cn(
                            ...
                            shouldHighlight && "ring-1 ring-indigo-500/30"
                        )}>
                            <div className="flex gap-4">
                                <div className="flex-shrink-0 w-8 h-8 rounded-xl flex items-center justify-center font-black text-xs border bg-slate-800 text-slate-500 border-slate-700">
                                    {q.id.replace(/.*-q/, '')}
                                </div>
                                <div className="flex-1 space-y-4">
                                    ...
                                    {reviewMode && showTranslation && q.translation && (
                                        <p className="text-sm font-medium text-indigo-400/80 -mt-2 animate-in fade-in slide-in-from-top-1 duration-300">
                                            {q.translation}
                                        </p>
                                    )}
