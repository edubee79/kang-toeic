...
export default function Part5ModeSelectionPage() {
    ...
    if (isMounted && !testSet) {
        return (
            <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center text-white">
                <h1 className="text-2xl font-bold mb-4">Initial Test Data Not Found</h1>
                <p className="text-slate-400 mb-8">Vol {vol} Test {testId} 데이터를 찾을 수 없습니다.</p>
                <Button onClick={() => router.push('/homework/part5-real')}>
                    돌아가기
                </Button>
            </div>
        );
    ...
    const title = testSet?.title || `Part 5 Real Test Vol ${vol} Test ${testId}`;

    const handleModeSelect = (mode: 'drill' | 'real') => {
        router.push(`/homework/part5-real/test/${vol}/${testId}?mode=${mode}`);
    };
