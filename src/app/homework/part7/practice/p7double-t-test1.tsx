        ...
        <div className="min-h-screen bg-gray-100 flex flex-col h-screen overflow-hidden font-sans">
            ...
            <main className="flex-1 flex overflow-hidden">
                ...
                <div className="w-[40%] flex flex-col h-full border-l border-gray-300">
                    ...
                    <div className={`${data.passages[2] ? 'h-[60%]' : 'h-full'} bg-white overflow-y-auto`}>
                        <div className="pb-20 p-6">
                            <h2 className="text-sm font-bold text-gray-500 uppercase tracking-widest mb-4 border-b border-gray-100 pb-2">
                                Questions {data.questions[0].id}-{data.questions[data.questions.length - 1].id}
                            </h2>

                            <div className="space-y-6"> {/* Reduced spacing between questions */}
                                {data.questions.map((q, idx) => {
                                    ...
                                    return (
                                        <div key={q.id} className={`transition p-3 rounded-lg ${showResults && isCorrect ? 'bg-green-50' : ''} ${showResults && isWrong ? 'bg-red-50' : ''}`}>
                                            <div className="flex gap-2 mb-2"> {/* Reduced gap */}
                                                <span className="font-bold text-blue-900 text-base">{qNum}.</span>
                                                <p className="font-medium text-gray-800 text-base leading-tight pt-0.5">
                                                    {q.text}
                                                </p>
                                            </div>
                                            ...
                                            <div className="space-y-1 pl-6"> {/* Reduced left padding and vertical spacing */}
                                                {q.options.map((option, optIdx) => {
                                                    ...
                                                    return (
                                                        <label
                                                            key={optIdx}
                                                            className={`flex items-start gap-2 p-2 rounded border cursor-pointer transition ${optionClass}`}
                                                        >
                                                            <div className={`w-4 h-4 rounded-full border flex items-center justify-center shrink-0 mt-0.5 ${isSelected ? 'border-blue-600 bg-blue-600' : 'border-gray-400 bg-white'
                                                                }`}>
