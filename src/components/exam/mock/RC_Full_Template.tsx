...
import { part5Questions } from '@/data/rc_part5';
import { part6Sets } from '@/data/rc_part6';
...
const RC_Full_Template: React.FC<Props> = ({ onFinish }) => {
    ...
    const nextSpread = () => {
        if (currentSpread < totalSpreads - 1) {
            ...
            if (window.confirm("Finish the Reading Test?")) {
                onFinish();
            }
        }
    };
        ...
        <>
            ...
            <div className="footer">
                <div style={{ position: 'absolute', left: '20px', display: 'flex', gap: '10px' }}>
                    <button onClick={prevSpread} disabled={currentSpread === 0}>Prev</button>
                    <button onClick={nextSpread}>
                        {currentSpread === totalSpreads - 1 ? 'Finish' : 'Next'}
                    </button>
                </div>
                <div>
...
export default RC_Full_Template;
