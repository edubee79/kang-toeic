...
export function calculateLevelScore(
    ...
    Object.entries(answers).forEach(([userQId, userAns]) => {
        ...
        if (userQId.startsWith('p1')) pKey = "p1";
            ...
            // Check qNum for p7s/p7m if necessary, but Level Test ID should be clear
            // For Level Test, we can try to find if it was in the single or multi list
    ...
    // Level Test Scoring: 100 questions total (50 LC + 50 RC)
    // Scale to 1000 for display? Or 990?
    // Let's use 10 points per question as per previous logic
